...
void fishy_gcdist(double *lon1, double *lon2, double *lat1, double *lat2,
               double *dist);
void fishy_dists(double *u, double *v, double *uout, double *vout,
              int *n, double *dists, int *lonlat);
void fishy_dists_NN(double *u1, double *v1, double *u2, double *v2,
                 int *n, double *dists, int *lonlat);
...
void fishy_dists(double *u, double *v, double *uout, double *vout,
  ...
  if (lonlat[0] == 0) {
    ...
    for (j = 0; j < N; j++) {
      fishy_gcdist(u+j, uout, v+j, vout, gc);
      dists[j] = gc[0];
    }
  }
}
...
void fishy_dists_NN(double *u1, double *v1, double *u2, double *v2,
  ...
  if (lonlat[0] == 0)
    ...
    for (j = 0; j < N; j++) {
      fishy_gcdist(u1+j, u2+j, v1+j, v2+j, gc);
      dists[j] = gc[0];
    }
  }
}
...
void fishy_lengths(double *u, double *v, int *n, double *lengths, int *lonlat) {
  ...
  if (lonlat[0] == 0)
    ...
    for (j=0; j < N-1; j++) {
      fishy_gcdist(u+j, u+j+1, v+j, v+j+1, gc);
      lengths[j] = gc[0];
    }
  }
